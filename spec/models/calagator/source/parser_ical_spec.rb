  ...
  describe Source::Parser::Ical, type: :model do
    ...
    describe 'with iCalendar events' do
      ...
      it 'should parse Google iCalendar feed with multiple events' do
        ...
        expect(event.start_time).to eq Time.parse('2007-10-24 18:30:00 PDT').in_time_zone
        expect(event.end_time).to eq Time.parse('2007-10-24 19:30:00 PDT').in_time_zone

        event = events[17]
        expect(event.title).to eq 'Code Sprint/Coding Dojo at CubeSpace'
        expect(event.description).to be_blank
        expect(event.start_time).to eq Time.parse('2007-10-17 19:00:00 PDT').in_time_zone
        expect(event.end_time).to eq Time.parse('2007-10-17 21:00:00 PDT').in_time_zone

        event = events.last
        expect(event.title).to eq 'Adobe Developer User Group'
        expect(event.description).to eq 'http://pdxria.com/'
        expect(event.start_time).to eq Time.parse('2007-01-16 17:30:00 PST').in_time_zone
    ...
    describe 'when importing events with non-local times' do
      it 'should store time ending in Z as UTC' do
        ...
        expect(event.start_time).to eq Time.parse('Thu Jul 01 08:00:00 +0000 2010').in_time_zone
      ...
      it 'should store time with TZID=GMT in UTC' do
        ...
        expect(events.size).to eq 1
        event = events.first
        expect(event.start_time).to eq Time.parse('Fri May 07 08:00:00 +0000 2020').in_time_zone
        expect(event.end_time).to eq Time.parse('Fri May 07 09:00:00 +0000 2020').in_time_zone
      end
    end
...
DESCRIPTION:Ayanami
DTSTART:#{(Time.now.in_time_zone - 1.year).strftime('%Y%m%d')}
DTSTAMP:040425
SEQ:0
END:VEVENT
BEGIN:VEVENT
...
DESCRIPTION:Soryu a
DTSTART:#{(Time.now.in_time_zone - 1.year).strftime('%Y%m%d')}
DTEND:#{(Time.now.in_time_zone + 1.year).strftime('%Y%m%d')}
DTSTAMP:040425
END:VEVENT
BEGIN:VEVENT
UID:Unit-04
...
DTSTART:#{Time.now.in_time_zone.strftime('%Y%m%d')}
DTEND:#{(Time.now.in_time_zone + 1.year).strftime('%Y%m%d')}
DTSTAMP:040425
END:VEVENT
BEGIN:VEVENT
