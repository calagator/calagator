class Organization < ActiveRecord::Base
  class SearchEngine
    class ApacheSunspot < Struct.new(:query, :opts)
      # Return an Array of non-duplicate Organization instances matching the search +query+..
      #
      # Options:
      ...
      # * :wifi => Require wifi
      # * :include_closed => Include closed organizations? Defaults to false.
      def self.search(*args)
        new(*args).search
      end

      def self.configure
        Organization.searchable do
      ...
      def configured?
        Organization.respond_to?(:solr_search)
      end
